...
enum EditorRowStyle {
    struct NumericOptions: OptionSet {
        ...
        static let standard: Self = [.decimalInput, .preferContinue]
...
//        static let  option   = Self(rawValue: 1 << 64)
    ...
    struct TextOptions: OptionSet {
        ...
        static let standard: Self = [.preferContinue]
...
extension View {
    ...
    func editorRow(decimalStyle options: EditorRowStyle.NumericOptions, onSubmit submit: (() -> Void)? = nil) -> some View {
        self.modifier(EditorRowDecimalStyle(configuration: options, onSubmit: submit))
    }
    
    @warn_unqualified_access
    func editorRow(textStyle options: EditorRowStyle.TextOptions, onSubmit submit: (() -> Void)? = nil) -> some View {
        self.modifier(EditorRowTextStyle(configuration: options, onSubmit: submit))
    }
}
