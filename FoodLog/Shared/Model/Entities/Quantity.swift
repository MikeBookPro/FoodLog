    ...
    func update<Value>(value: Value, for keyPath: KeyPath<Quantity, Value>) -> Self {
      if keyPath == \.measurement.value, let val = value as? Double {
        return .init(
          quantity: Quantity(
            ...
            measurement: .init(value: val, unit: quantity.measurement.unit)
          )
        )
      }
      ...
      if keyPath == \.measurement.unit, let dim = value as? Dimension {
        return .init(
          quantity: Quantity(
            identifier: quantity.identifier,
            measurement: .init(value: quantity.measurement.value, unit: dim)
          )
        )
      }
      ...
      if keyPath == \.measurement, let measure = value as? Measurement<Dimension> {
        return .init(
          quantity: Quantity(identifier: quantity.identifier, measurement: measure)
        )
      }
